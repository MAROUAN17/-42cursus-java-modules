...
public class Transaction {
    ...
    Transaction(UUID id, User sender, User recipient, TransferCategory transferCategory, Integer amount) {
        if (transferCategory == TransferCategory.CREDITS && amount > 0) {
            throw new IllegalArgumentException("Credits amount should be negative");
        }
        if (transferCategory == TransferCategory.DEBITS && amount < 0) {
            throw new IllegalArgumentException("Debits amount should be positive");
        }
    ...
    Transaction(User sender, User recipient, TransferCategory transferCategory, Integer amount) {
        this(UUID.randomUUID(), sender, recipient, transferCategory, amount);
    }
