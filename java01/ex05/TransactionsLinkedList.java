...
public class TransactionsLinkedList implements TransactionsList {
    ...
    public Transaction get(UUID transactionID) {
        ...
        while(curr != null) {
            if (curr.data.getID().equals(transactionID)) return curr.data;
            curr = curr.next;
        }
        return null;
    }
    ...
    public void removeTransaction(UUID id) {
        if (head == null) return;
            ...
            size--;
            if (id.equals(head.data.getID()))
                head = head.next;
            else
            {
                NodeTransaction curr = head;
                while (curr.next != null && curr.next.data.getID() != id)
                    curr = curr.next;
                if (curr.next != null && curr.next.data.getID() != id)
